...
import { FacebookGlyph, InstagramGlyph, MapPinIcon, TikTokGlyph } from "@/components/icons";
  ...
  return (
    ...
    >
      ...
      <span className="text-[13.5px] font-medium text-ink">
        {pending ? t("contact.socialComingSoon", { label }) : label}
      </span>
    </a>
  );
}
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-6 pb-28 pt-32 md:px-10 md:pt-40">
      <div className="max-w-2xl">
        <p className="eyebrow mb-5 text-[11px] font-medium uppercase text-wood-dark">{t("contact.eyebrow")}</p>
        <h1 className="font-display text-4xl text-charcoal md:text-5xl">{t("contact.title")}</h1>
        <p className="mt-6 text-[15px] leading-relaxed text-charcoal-soft/75">{t("contact.intro")}</p>
      </div>
      ...
      <div className="mt-16 grid grid-cols-1 gap-10 lg:grid-cols-2">
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <div>
              <p className="font-display text-[15px] font-semibold">{t("contact.chatWhatsapp")}</p>
              <p className="text-[13px] text-white/80">{t("contact.fastest")}</p>
            </div>
          </a>
      ...
      <div className="mt-24">
        <div className="mb-8 flex items-center gap-2">
          ...
          <h2 className="font-display text-2xl text-charcoal">{t("contact.followTiktok")}</h2>
        </div>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((n) => (
              ...
              >
                ...
                <p className="px-6 text-[12.5px] text-charcoal-soft/70">{t("contact.videoComingSoon", { n })}</p>
              </div>
            ))}
          </div>
        )}
